function alertBox(msg, btn_num, btn_left, btn_right, fn_left, fn_right) {
  if (!$('.alert_bg').length && !$('.alert_box').length) {
    var html = '<div class="alert_bg" id="alert_bg"></div>' + '<div class="alert_box">' + '<div class="alert_msg">信息</div>' + '<div class="alert_btn"></div>' + '</div>';
    $(html).appendTo($('body'));
...
function uploadImg() {
  /*for(var i=6;i>0;i--){
	  var v=new UploadImg({
		url:'/upimgs',
		container: "#pic-list"+i,
		countBox: "#count-box"+i,
		fileInput: "#fileImg"+i,
		hiddenValue: "#hiddenValue"+i
	  })
  }*/
  var a=new UploadImg({
	url:'/upimgs'
